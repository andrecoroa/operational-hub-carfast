...
> provas ou migração. Ver `STORAGE_PRESEED_DELTA_MIGRATION_RUNBOOK.md` para o percurso
> convencional ativo.
...
The only Render command is produced by `scripts.integral_render_contract` and checked
into `render.integral.yaml`, which invokes `scripts.integral_render_entrypoint` under
