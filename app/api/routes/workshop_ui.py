...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--text);font-size:14px;letter-spacing:0}.app{display:grid;grid-template-columns:248px minmax(0,1fr);min-height:100vh}aside{background:#10202c;color:#d9e7ef;padding:20px 14px}.brand{font-weight:800;font-size:18px;padding:8px 10px 20px;color:#fff}.nav{display:grid;gap:4px}.nav a{min-height:36px;padding:8px 10px;border-radius:8px;color:#d9e7ef;text-decoration:none;font-weight:650}.nav .sub{margin-left:18px;color:#b6cad5}.nav .active{background:#f4ebe7;color:#7d2f1f}main{padding:22px 28px}h1{margin:0 0 4px;font-size:25px}.subtitle{margin:0;color:var(--muted)}.topbar{display:flex;justify-content:space-between;gap:16px;align-items:start;margin-bottom:18px}.top-actions{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:8px}.button{display:inline-flex;align-items:center;justify-content:center;min-height:40px;border-radius:8px;padding:9px 14px;background:var(--brand);color:#fff;text-decoration:none;font-weight:800}.button.secondary{background:#fff;color:var(--text);border:1px solid var(--line)}.list{display:grid;gap:10px}.process-card{display:grid;grid-template-columns:minmax(160px,220px) minmax(0,1fr) auto;gap:18px;align-items:center;background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:14px 16px}.plate{font-size:22px;font-weight:900}.small{font-size:12px;color:var(--muted);line-height:1.4}.title{font-size:16px;font-weight:900;margin-bottom:6px}.meta{display:flex;flex-wrap:wrap;gap:8px}.chip{display:inline-flex;align-items:center;border-radius:999px;min-height:26px;padding:4px 10px;background:#eef1f3;color:var(--muted);font-size:12px;font-weight:850}.chip.done{color:var(--green);background:var(--green-soft)}.chip.progress{color:var(--blue);background:var(--blue-soft)}.chip.review{color:var(--amber);background:var(--amber-soft)}.chip.danger{color:var(--red);background:var(--red-soft)}.empty{background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:24px;color:var(--muted)}@media(max-width:900px){.app{grid-template-columns:1fr}aside{display:none}main{padding:18px 16px}.topbar{display:grid}.process-card{grid-template-columns:1fr}.process-card .button{width:100%}}
  </style>
</head>
<body>
  <div class="app">
  ...
  <script>
    ...
    function card(p) {
      ...
      return `<article class="process-card">
        ...
        <div><div class="title">${service}</div><div class="small">${vehicleName(v)}</div><div class="meta"><span class="chip ${status[1]}">${status[0]}</span><span class="chip">${phase}</span><span class="chip">Prioridade ${PRIORITY[p.priority] || p.priority || "-"}</span></div></div>
