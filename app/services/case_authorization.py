...
from app.models.cases import OperationalCase
...
ACTION_PERMISSIONS = {
    ...
    "delegate": "processes.delegate",
    "validate": "processes.validate",
...
def _target_unit_id(db: Session, link_type: str, target: object) -> int | None:
    ...
    if link_type == "workshop":
        ...
        return vehicle.current_location_id if vehicle else None
    return None  # Email has no trustworthy organizational scope in T1.
...
def can_access_link(
    ...
    if "admin.manage" in permissions:
        return True
    allowed = rule[1].get("write" if action != "read" else "read")
    if not allowed or not permissions.intersection(allowed):
        return False
